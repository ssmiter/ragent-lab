 ...
 * </pre>
 *
 * <p>这是 Agent Loop 的本质：循环 → 调模型 → 检查 tool_use → 执行工具 → 拼结果 → 下一轮。</p>
 */
@Slf4j
    ...
    public AgentLoopResult run(String userQuery) {
        ...
        try {
            ...
            while (true) {
                ...
                log.info("LLM 响应长度: {} 字符", response.length());
                ...
                if (toolCalls.isEmpty()) {
                    ...
                    log.info("模型返回最终回答: {}", finalContent.length() > 200
                            ? finalContent.substring(0, 200) + "..." : finalContent);
